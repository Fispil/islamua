...
// ── Low-pass filter coefficient (0 = no filtering, 1 = no movement) ────────
const ALPHA = 0.15;   // higher = smoother but more lag; 0.15 feels natural
...
export default function QiblaScreen() {
  ...
  // Refs for raw sensor values — updated at 20Hz without triggering re-render
  const magRef   = useRef({ x: 0, y: 0, z: 0 });
  const accelRef = useRef({ x: 0, y: 0, z: -1 }); // default: phone flat face-up
  const smoothRef = useRef(0); // smoothed heading
  ...
  useEffect(() => {
    // 20 Hz update interval — fast enough, not battery-draining
  ...
  function computeHeading() {
    ...
    let raw = Math.atan2(-Yh, Xh) * (180 / Math.PI);
    if (raw < 0) raw += 360;

    // ── Low-pass filter to smooth out jitter ─────────────────────────────────
    // Handle 0°/360° wraparound correctly
    let diff = raw - smoothRef.current;
    if (diff > 180)  diff -= 360;
    if (diff < -180) diff += 360;
    const smooth = (smoothRef.current + ALPHA * diff + 360) % 360;
  ...
  const aligned = Math.abs((needle + 360) % 360) < 5
               || Math.abs((needle + 360) % 360) > 355;
  const dist = location ? Math.round(distanceToMecca(location.lat, location.lng)) : null;

  const cx = 150, cy = 150, R = 130;
  const rad  = (needle * Math.PI) / 180;
  const tipX = cx + Math.sin(rad) * (R - 20);
  const tipY = cy - Math.cos(rad) * (R - 20);
  const tailX = cx - Math.sin(rad) * 55;
  const tailY = cy + Math.cos(rad) * 55;
        ...
        <View style={s.content}>
          ...
          <View style={s.infoCard}>
            <View style={{ flex: 1, gap: 2 }}>
              ...
              <Text style={s.infoCity}>
                {location ? `${location.city}, ${location.country}` : '--'}
              </Text>
              {location && (
                <Text style={s.infoCoords}>
                  {location.lat.toFixed(4)}{t('degreesN')}, {location.lng.toFixed(4)}{t('degreesE')}
                </Text>
              )}
            </View>
          ...
          <View style={s.compassWrap}>
            <Svg width={300} height={300}>
              ...
              <Circle
                cx={cx} cy={cy} r={R}
                fill="none"
                stroke={aligned ? Colors.green : Colors.goldBorder}
                strokeWidth={1.5}
              />
              {/* Tick marks */}
              {Array.from({ length: 36 }).map((_, i) => {
                ...
                return (
                  <Line
                    ...
                    stroke={Colors.borderSoft} strokeWidth={0.7}
                  />
                );
              })}
              {/* Cardinal labels */}
              {(['N', 'E', 'S', 'W'] as const).map((d, i) => {
                ...
                return (
                  <SvgText
                    ...
                    y={cy - Math.cos(a) * (R - 22) + 4}
                    fontSize={11}
                    fill={d === 'N' ? Colors.gold : Colors.textSecondary}
              ...
              <Line
                ...
                stroke={aligned ? Colors.green : Colors.gold}
              ...
              <SvgText
                x={cx} y={cy + R + 20}
                fontSize={12} fill={Colors.textSecondary} textAnchor="middle"
              >
                {Math.round(heading)}°
              </SvgText>
            </Svg>
