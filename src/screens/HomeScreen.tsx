...
import { enableNotifications, disableNotifications, togglePrayerNotif } from '../services/notificationServiceOld';
...
export default function HomeScreen() {
  ...
  const { prayers, dayData, location, nextPrayer, currentPrayer, countdown, progress, loading, error, notifEnabled, prayerNotifs, refresh } = usePrayerTimes();
  ...
  const handleEnableNotif = async () => {
    ...
    if (!ok) Alert.alert(t('permissionRequired'), t('permissionMsg'), [{ text:t('ok') }]);
    else setShowBanner(false);
    await refresh();
  };

  const handleToggle = async (name: string) => {
    await togglePrayerNotif(name, !(prayerNotifs[name]??false), prayers);
    await refresh();
  };
  ...
  return (
    <SafeAreaView style={s.safe} edges={['top']}>
      <ScrollView style={s.scroll} showsVerticalScrollIndicator={false}
        ...
        {error && (
          <View style={s.errorBanner}><Text style={s.errorText}>⚠️ {error}</Text></View>
        )}
        {showBanner && !notifEnabled && (
          <NotifBanner onEnable={handleEnableNotif} onDismiss={() => setShowBanner(false)} />
        )}
        {nextPrayer && (
          <NextPrayerCard nextPrayer={nextPrayer} currentPrayer={currentPrayer} countdown={countdown} progress={progress} />
        )}

        <SectionLabel text={t('todaysPrayers')} />
        <View style={s.list}>
          {prayers.map(prayer => (
            <PrayerTile key={prayer.name} prayer={prayer}
