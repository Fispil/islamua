...
// Islam UA — two widgets: NextPrayer + Calendar (all 5 prayers)
// Real-time: WidgetKit timeline with 60 entries ahead (one per minute)
...
// ══════════════════════════════════════════════════════════════════════
// NEXT PRAYER WIDGET — with live 1-minute timeline
// ══════════════════════════════════════════════════════════════════════
...
struct NextProvider: TimelineProvider {
    ...
    private func load() -> NextPrayerData {
        guard let ud = UserDefaults(suiteName: APP_GROUP),
              ...
              let data = try? JSONDecoder().decode(NextPrayerData.self, from: raw) else {
            return .placeholder
        }
        return data
    }

    // Recompute countdown + progress for a specific moment in time
    private func recomputeForTime(base: NextPrayerData, currentTime: Date) -> NextPrayerData {
        guard let nextDate = parseHHmm(base.time12h, relativeTo: currentTime),
              let curDate = parseHHmm(base.currentPrayerTime, relativeTo: currentTime) else {
            return base
        }
        ...
        // Progress = elapsed from current → next
    ...
    private func parseHHmm(_ s: String, relativeTo ref: Date) -> Date? {
        ...
        // If time already passed today relative to ref, move to tomorrow
...
// ── Next Prayer Widget UI ──────────────────────────────────────────────
struct NextPrayerWidgetView: View {
    @Environment(\.widgetFamily) var family
    let entry: NextPrayerEntry
    var body: some View {
        switch family {
        case .systemSmall:            SmallNextPrayerView(d: entry.data)
        case .systemMedium:           MediumNextPrayerView(d: entry.data)
        case .accessoryRectangular:   LockScreenNextPrayerView(d: entry.data)
        case .accessoryCircular:
            VStack(spacing: 1) {
                Text(entry.data.arabicName).font(.system(size: 9))
                Text(entry.data.time12h).font(.system(size: 14, weight: .semibold))
            }.widgetAccentable()
        default: MediumNextPrayerView(d: entry.data)
        }
    }
}

struct MediumNextPrayerView: View {
    let d: NextPrayerData
    var body: some View {
        ZStack {
            LinearGradient(colors: [cCardHi, cCard], startPoint: .topLeading, endPoint: .bottomTrailing)
            VStack(alignment: .leading, spacing: 0) {
                HStack(spacing: 7) {
                    Circle().fill(cGreen).frame(width: 7, height: 7)
                    Text("NEXT PRAYER").font(.system(size: 10, weight: .bold))
                        .foregroundColor(cMuted).tracking(1.2)
                    Spacer()
                }.padding(.bottom, 12)
...
struct SmallNextPrayerView: View {
    let d: NextPrayerData
    var body: some View {
        ZStack {
            LinearGradient(colors: [cCardHi, cCard], startPoint: .topLeading, endPoint: .bottomTrailing)
            VStack(alignment: .leading, spacing: 0) {
                HStack(spacing: 5) {
                    ...
                    Text("NEXT PRAYER").font(.system(size: 8, weight: .bold))
                        .foregroundColor(cMuted).tracking(0.9)
                }
                Spacer()
                Text(d.prayerName).font(.system(size: 22, weight: .bold, design: .serif)).foregroundColor(cWhite)
...
struct LockScreenNextPrayerView: View {
    let d: NextPrayerData
    var body: some View {
        HStack(spacing: 8) {
...
struct CalendarProvider: TimelineProvider {
    func placeholder(in _: Context) -> CalendarEntry { CalendarEntry(date: .now, data: .placeholder) }
    ...
    func getTimeline(in _: Context, completion: @escaping (Timeline<CalendarEntry>) -> Void) {
        ...
        // Reload every 5 minutes — calendar doesn't need per-second updates
    ...
    private func load() -> AllPrayersData {
        guard let ud = UserDefaults(suiteName: APP_GROUP),
              ...
              let data = try? JSONDecoder().decode(AllPrayersData.self, from: raw) else {
            return .placeholder
        }
        return data
    }
    ...
    var body: some View {
        ZStack {
            ...
            VStack(alignment: .leading, spacing: 0) {
                // Header
                HStack {
                    Text(entry.data.city).font(.system(size: 13, weight: .bold)).foregroundColor(cWhite)
                    Spacer()
...
// ══════════════════════════════════════════════════════════════════════
// WIDGET BUNDLE — register both widgets
// ══════════════════════════════════════════════════════════════════════
